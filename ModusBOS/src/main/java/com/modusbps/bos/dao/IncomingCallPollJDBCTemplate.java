...
public class IncomingCallPollJDBCTemplate implements IncomingCallPollDAO {
	...
	public void createIncomingCallPoll(IncomingCallPoll incomingCallPoll) {
		String sql = "INSERT INTO incoming_call_poll(id,data,description,type) VALUES (?,?,?,?)";
		
		jdbcTemplateObject.update(sql, incomingCallPoll.getId(), incomingCallPoll.getData(), incomingCallPoll.getDescription(), incomingCallPoll.getType());
	}
